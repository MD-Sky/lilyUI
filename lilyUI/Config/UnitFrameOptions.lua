...
local function GetUnitDB(unit)
    ...
    if not LilyUI.db.profile.unitFrames then
        LilyUI.db.profile.unitFrames = {}
    end
    if not LilyUI.db.profile.unitFrames[dbUnit] then
        LilyUI.db.profile.unitFrames[dbUnit] = {}
    end
    return LilyUI.db.profile.unitFrames[dbUnit]
end
...
local function UpdateUnitFrame(unit)
    if LilyUI.UnitFrames then
        ...
        LilyUI.UnitFrames:ResolveMedia()
        
        -- For boss frames, update all boss frames (boss1-boss8)
        if unit == "boss" then
            for i = 1, 8 do
                LilyUI.UnitFrames:UpdateUnitFrame("boss" .. i)
            end
        else
            -- Update the specific unit frame
            LilyUI.UnitFrames:UpdateUnitFrame(unit)
        end
    end
end
        ...
        args = {
            ...
            width = {
                ...
                set = function(_, val)
                    ...
                    if LilyUI.UnitFrames and LilyUI.UnitFrames.LayoutBossFrames then
                        LilyUI.UnitFrames:LayoutBossFrames()
                    end
                end,
            },
            height = {
        ...
        return {
            ...
            args = {
                ...
                previewMode = unit == "boss" and {
                    ...
                    width = "full",
                    get = function()
                        return LilyUI.UnitFrames and LilyUI.UnitFrames.BossPreviewMode or false
                    end,
                    set = function(_, val)
                        if LilyUI.UnitFrames then
                            if val then
        ...
        return {
            ...
            args = {
                    ...
                    set = function(_, val)
                        ...
                        if LilyUI.UnitFrames then
                            C_Timer.After(0.1, function()
                                LilyUI.UnitFrames:HookAnchorFrames()
                            end)
                        end
                    end,
                },
                anchorFrame = unit ~= "boss" and {
                    type = "input",
                    name = "Anchor Frame",
                    desc = "Frame name to anchor to (e.g., EssentialCooldownViewer, LilyUI_Player, LilyUI_Target, UIParent)",
                    order = 22,
                    width = "full",
                    get = function()
            ...
            args = {
                ...
                cooldownFontSize = {
                    ...
                    set = function(_, val)
                        ...
                        local unitFrameName = (unit == "player") and "LilyUI_Player" or (unit == "focus") and "LilyUI_Focus" or "LilyUI_Target"
        ...
        return {
            ...
            args = {
                texture = {
                    ...
                    get = function()
                        return DB.AbsorbBar.Texture or "Lily"
                    end,
                    set = function(_, val)
                ...
                args = {
                    ...
                    enabled = {
                        ...
                        get = function()
                            ...
                            return LilyUI.db.profile.unitFrames.enabled or false
                        end,
                        set = function(_, val)
                            if not LilyUI.db.profile.unitFrames then
                                LilyUI.db.profile.unitFrames = {}
                            end
                            LilyUI.db.profile.unitFrames.enabled = val
                            if LilyUI.UnitFrames then
                                if val then
                                    LilyUI.UnitFrames:Initialize()
                                else
                                    -- Hide all frames when disabled
                                    for unit in pairs({player = true, target = true, targettarget = true, pet = true, focus = true}) do
                                        local frameName = "LilyUI_" .. unit:gsub("^%l", string.upper):gsub("targettarget", "TargetTarget")
                    ...
                    foregroundTexture = {
                        ...
                        set = function(_, val)
                            ...
                            if LilyUI.UnitFrames then
                                LilyUI.UnitFrames:ResolveMedia()
                                LilyUI.UnitFrames:RefreshFrames()
                            end
                        end,
                    },
                    backgroundTexture = {
                    ...
                    font = {
                        ...
                        get = function()
                            return LilyUI.db.profile.general.globalFont or "Friz Quadrata TT"
                        end,
                        set = function(_, val)
                    ...
                    mouseoverToggle = {
                        ...
                        desc = "Show the custom mouseover glow on all LilyUI unit frames",
                        order = 35,
                        width = "full",
                        get = function()
                            local db = LilyUI.db.profile.unitFrames
                        ...
                        set = function(_, val)
                            ...
                            if not db then
                                LilyUI.db.profile.unitFrames = {}
                                db = LilyUI.db.profile.unitFrames
                            end
                            if not db.General then db.General = {} end
