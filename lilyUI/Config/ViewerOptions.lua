...
local function CreateViewerOptions(viewerKey, displayName, order)
        ...
        args = {
            ...
            rowLimit = {
                ...
                set = function(_, val)
                    ...
                    if viewer then
                        ...
                        if LilyUI.IconViewers and LilyUI.IconViewers.ApplyViewerLayout then
                            LilyUI.IconViewers:ApplyViewerLayout(viewer)
                        end
                    end
                    if LilyUI.RefreshViewers then
                        LilyUI:RefreshViewers()
            ...
            iconSize = {
                ...
                set = function(_, val)
                    ...
                    local viewer = _G[viewerKey]
                    if viewer and LilyUI.IconViewers then
                        if LilyUI.IconViewers.SkinAllIconsInViewer then
                            LilyUI.IconViewers:SkinAllIconsInViewer(viewer)
                        end
                        if LilyUI.IconViewers.ApplyViewerLayout then
                            LilyUI.IconViewers:ApplyViewerLayout(viewer)
                        end
                    end
                    if LilyUI.RefreshViewers then
            ...
            rowIconSize1 = {
                ...
                get = function()
                    local sizes = LilyUI.db.profile.viewers[viewerKey].rowIconSizes
            ...
            spacing = {
                ...
                set = function(_, val)
                    ...
                    -- Force immediate layout update
                    local viewer = _G[viewerKey]
                    if viewer and LilyUI.IconViewers and LilyUI.IconViewers.ApplyViewerLayout then
                        LilyUI.IconViewers:ApplyViewerLayout(viewer)
                    end
                    if LilyUI.RefreshViewers then
                        LilyUI:RefreshViewers()
            ...
            primaryDirection = {
                ...
                set = function(_, val)
                    if not LilyUI.db.profile.viewers[viewerKey] then
                        LilyUI.db.profile.viewers[viewerKey] = {}
                    end
            ...
            cooldownFontSize = {
                ...
                min = 8, max = 48, step = 1,
                get = function()
                    return LilyUI.db.profile.viewers[viewerKey].cooldownFontSize or 
                ...
                set = function(_, val)
                    ...
                    LilyUI.db.profile.viewers[viewerKey].cooldownFontSize = val
            ...
            cooldownTextColor = {
                ...
                hasAlpha = true,
                get = function()
                    local c = LilyUI.db.profile.viewers[viewerKey].cooldownTextColor or 
                ...
                set = function(_, r, g, b, a)
                    ...
                    LilyUI.db.profile.viewers[viewerKey].cooldownTextColor = {r, g, b, a or 1}
                    -- Refresh all cooldown fonts
                    if LilyUI.ApplyGlobalFont then
                        LilyUI:ApplyGlobalFont()
                    end
            ...
            cooldownShadowOffsetX = {
                ...
                min = -5, max = 5, step = 1,
                get = function()
                    return LilyUI.db.profile.viewers[viewerKey].cooldownShadowOffsetX or 
                           (LilyUI.db.profile.viewers.general and LilyUI.db.profile.viewers.general.cooldownShadowOffsetX) or 1
                end,
                set = function(_, val)
                    if not LilyUI.db.profile.viewers[viewerKey] then
                        LilyUI.db.profile.viewers[viewerKey] = {}
                    end
                    LilyUI.db.profile.viewers[viewerKey].cooldownShadowOffsetX = val
                    -- Refresh all cooldown fonts
                    if LilyUI.ApplyGlobalFont then
                        LilyUI:ApplyGlobalFont()
                    end
            ...
            cooldownShadowOffsetY = {
                ...
                min = -5, max = 5, step = 1,
                get = function()
                    return LilyUI.db.profile.viewers[viewerKey].cooldownShadowOffsetY or 
                           (LilyUI.db.profile.viewers.general and LilyUI.db.profile.viewers.general.cooldownShadowOffsetY) or -1
                end,
                set = function(_, val)
                    if not LilyUI.db.profile.viewers[viewerKey] then
                        LilyUI.db.profile.viewers[viewerKey] = {}
                    end
                    LilyUI.db.profile.viewers[viewerKey].cooldownShadowOffsetY = val
                    -- Refresh all cooldown fonts
                    if LilyUI.ApplyGlobalFont then
                        LilyUI:ApplyGlobalFont()
                    end
    ...
    if viewerKey == "EssentialCooldownViewer" or viewerKey == "UtilityCooldownViewer" then
        ...
        ret.args.showKeybinds = {
            ...
            width = "full",
            get = function()
                return LilyUI.db.profile["cooldownManager_showKeybinds_" .. viewerSettingName] or false
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_showKeybinds_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:OnSettingChanged(viewerSettingName)
                end
            end,
        }
        ...
        ret.args.keybindFont = {
            ...
            get = function()
                local font = LilyUI.db.profile["cooldownManager_keybindFontName_" .. viewerSettingName]
                if not font or font == "" then
                    font = LilyUI.db.profile.cooldownManager_keybindFontName or "Friz Quadrata TT"
                end
                return font
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_keybindFontName_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
            disabled = function()
                return not (LilyUI.db.profile["cooldownManager_showKeybinds_" .. viewerSettingName] or false)
            end,
        }
        ...
        ret.args.keybindFontSize = {
            ...
            step = 1,
            get = function()
                return LilyUI.db.profile["cooldownManager_keybindFontSize_" .. viewerSettingName] or 14
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_keybindFontSize_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
        ...
        ret.args.keybindFontColor = {
            ...
            hasAlpha = true,
            get = function()
                local c = LilyUI.db.profile["cooldownManager_keybindFontColor_" .. viewerSettingName]
                if not c then
                    c = LilyUI.db.profile.cooldownManager_keybindFontColor or {1, 1, 1, 1}
                end
                return c[1], c[2], c[3], c[4] or 1
            end,
            set = function(_, r, g, b, a)
                LilyUI.db.profile["cooldownManager_keybindFontColor_" .. viewerSettingName] = {r, g, b, a or 1}
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
        ...
        ret.args.keybindAnchor = {
            ...
            get = function()
                return LilyUI.db.profile["cooldownManager_keybindAnchor_" .. viewerSettingName] or "TOPRIGHT"
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_keybindAnchor_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
        ...
        ret.args.keybindOffsetX = {
            ...
            step = 1,
            get = function()
                return LilyUI.db.profile["cooldownManager_keybindOffsetX_" .. viewerSettingName] or -3
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_keybindOffsetX_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
        ...
        ret.args.keybindOffsetY = {
            ...
            step = 1,
            get = function()
                return LilyUI.db.profile["cooldownManager_keybindOffsetY_" .. viewerSettingName] or -3
            end,
            set = function(_, val)
                LilyUI.db.profile["cooldownManager_keybindOffsetY_" .. viewerSettingName] = val
                if LilyUI.Keybinds then
                    LilyUI.Keybinds:ApplyKeybindSettings(viewerKey)
                end
            end,
        ...
        ret.args.hideAuraSwipe = {
            ...
            width = "full",
            get = function()
                return LilyUI.db.profile.viewers[viewerKey].ignoreAuraOverride or false
            end,
            set = function(_, val)
                if not LilyUI.db.profile.viewers[viewerKey] then
                    LilyUI.db.profile.viewers[viewerKey] = {}
                end
                LilyUI.db.profile.viewers[viewerKey].ignoreAuraOverride = val
                -- Refresh all icons to apply the change
                if LilyUI.AuraOverride and LilyUI.AuraOverride.RefreshViewer then
                    local viewer = _G[viewerKey]
                    if viewer then
                        LilyUI.AuraOverride:RefreshViewer(viewer)
                    end
                end
                if LilyUI.RefreshViewers then
                    LilyUI:RefreshViewers()
