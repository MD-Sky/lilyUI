...
if not LilyUI then
	error("LilyUI not found! PixelPerfect.lua must load after Main.lua")
end
...
function LilyUI:RefreshGlobalFX()
...
function LilyUI:IsEyefinity(width, height)
	if LilyUI.db.profile.general.eyefinity and width >= 3840 then
...
function LilyUI:IsUltrawide(width, height)
	if LilyUI.db.profile.general.ultrawide and width >= 2560 then
...
function LilyUI:UIMult()
	LilyUI.mult = LilyUI.perfect
end

function LilyUI:PixelBestSize()
	return max(0.4, min(1.15, LilyUI.perfect))
end

function LilyUI:PixelScaleChanged(event)
	if event == 'UI_SCALE_CHANGED' then
		LilyUI.physicalWidth, LilyUI.physicalHeight = GetPhysicalScreenSize()
		LilyUI.resolution = format('%dx%d', LilyUI.physicalWidth, LilyUI.physicalHeight)
		LilyUI.perfect = 768 / LilyUI.physicalHeight
	end
...
function LilyUI:ScaleBorder(borderSize)
