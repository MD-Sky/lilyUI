...
local function SetTargetCastBarColor(state)
    local cfg = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.targetCastBar
    if not cfg then return end

    local bar = LilyUI.targetCastBar
...
function CastBars:GetTargetCastBar()
    if LilyUI.targetCastBar then return LilyUI.targetCastBar end
    ...
    local tex = LilyUI:GetTexture(LilyUI.db.profile.targetCastBar.texture)
    ...
    if targetSpellbar and not targetSpellbar.__nephuiHooked then
        ...
        targetSpellbar:HookScript("OnShow", function(self)
            ...
            if not cfg or not cfg.enabled then
                if LilyUI.targetCastBar then LilyUI.targetCastBar:Hide() end
                return
            end
        ...
        targetSpellbar:HookScript("OnHide", function()
            if LilyUI.targetCastBar then
                LilyUI.targetCastBar:Hide()
            end
        end)
...
LilyUI.GetTargetCastBar = function(self) return CastBars:GetTargetCastBar() end
LilyUI.UpdateTargetCastBarLayout = function(self) return CastBars:UpdateTargetCastBarLayout() end
LilyUI.HookTargetAndFocusCastBars = function(self) return CastBars:HookTargetAndFocusCastBars() end
