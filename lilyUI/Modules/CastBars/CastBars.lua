...
-- Create namespace
LilyUI.CastBars = LilyUI.CastBars or {}
...
local function CreateBorder(frame)
	...
	local borderSize = (LilyUI.ScaleBorder and LilyUI:ScaleBorder(1)) or math.floor((LilyUI:Scale(1) or 1) + 0.5)
...
local function CastBar_OnUpdate(frame, elapsed)
    ...
    if frame.timeText then
        ...
        local cfg
        if frame == LilyUI.castBar then
            cfg = LilyUI.db.profile.castBar
        elseif frame == LilyUI.targetCastBar then
            cfg = LilyUI.db.profile.targetCastBar
        elseif frame == LilyUI.focusCastBar then
            cfg = LilyUI.db.profile.focusCastBar
        elseif LilyUI.bossCastBars then
            -- Check if this is a boss cast bar
            for _, bossBar in pairs(LilyUI.bossCastBars) do
                if frame == bossBar then
                    cfg = LilyUI.db.profile.bossCastBar
                    break
                end
            end
        end
        ...
        if cfg and cfg.showTimeText ~= false then
            ...
            if cfg == LilyUI.db.profile.bossCastBar then
...
function CastBars:Initialize()
    ...
    LilyUI:RegisterEvent("UNIT_SPELLCAST_START", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastStart then
            self:OnPlayerSpellcastStart(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_STOP", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastStop then
            self:OnPlayerSpellcastStop(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_FAILED", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastStop then
            self:OnPlayerSpellcastStop(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_INTERRUPTED", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastStop then
            self:OnPlayerSpellcastStop(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_CHANNEL_START", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastChannelStart then
            self:OnPlayerSpellcastChannelStart(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_CHANNEL_STOP", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastStop then
            self:OnPlayerSpellcastStop(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_CHANNEL_UPDATE", function(_, unit, castGUID, spellID)
    ...
    LilyUI:RegisterEvent("UNIT_SPELLCAST_EMPOWER_START", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastEmpowerStart then
            self:OnPlayerSpellcastEmpowerStart(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_EMPOWER_UPDATE", function(_, unit, castGUID, spellID)
        if unit == "player" and self.OnPlayerSpellcastEmpowerUpdate then
            self:OnPlayerSpellcastEmpowerUpdate(unit, castGUID, spellID)
        end
    end)
    
    LilyUI:RegisterEvent("UNIT_SPELLCAST_EMPOWER_STOP", function(_, unit, castGUID, spellID)
...
function CastBars:ShowTestFocusCastBar()
    ...
    local cfg = LilyUI.db.profile.focusCastBar
    local font = LilyUI:GetGlobalFont()
    bar.spellName:SetFont(font, cfg.textSize or 10, "OUTLINE")
    bar.spellName:SetShadowOffset(0, 0)
...
LilyUI.ShowTestCastBar = function(self) return CastBars:ShowTestCastBar() end
LilyUI.ShowTestTargetCastBar = function(self) return CastBars:ShowTestTargetCastBar() end
LilyUI.ShowTestFocusCastBar = function(self) return CastBars:ShowTestFocusCastBar() end
