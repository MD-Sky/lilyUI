...
LilyUI.ActionBarGlow = LilyUI.ActionBarGlow or {}
local ActionBarGlow = LilyUI.ActionBarGlow
...
local function GetActionBarGlowSettings()
    local settings = LilyUI.db.profile.actionBars.procGlow
...
local function ApplyLibCustomGlow(button, settings, spellID)
    ...
    local glowKey = "_LilyUIActionBarGlow"
...
function ActionBarGlow:StopGlow(button)
    ...
    if LCG then
        pcall(LCG.PixelGlow_Stop, button, "_LilyUIActionBarGlow")
        pcall(LCG.AutoCastGlow_Stop, button, "_LilyUIActionBarGlow")
        pcall(LCG.ProcGlow_Stop, button, "_LilyUIActionBarGlow")
...
function ActionBarGlow:Initialize()
    ...
    C_Timer.After(1, function()
        local function HookButtonAlert(button)
            ...
            if alert then
                -- Hook Show method
                if not alert._LilyUIShowHooked then
                    ...
                    alert._LilyUIShowHooked = true
                end
                
                -- Hook Hide method
                if not alert._LilyUIHideHooked then
