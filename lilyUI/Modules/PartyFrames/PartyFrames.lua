...
LilyUI.CompactFrames = LilyUI.CompactFrames or {}
local Engine = LilyUI.CompactFrames

local PartyFrames = {}
LilyUI.PartyFrames = PartyFrames

local pendingResizeFrames = {}
local ShouldHidePartyPlayerEntry

local ROLE_TEXTURES = {
    TANK = "Interface\\AddOns\\LilyUI\\Media\\Tank.tga",
    HEALER = "Interface\\AddOns\\LilyUI\\Media\\Healer.tga",
    DAMAGER = "Interface\\AddOns\\LilyUI\\Media\\DPS.tga",
}

local HIGHLIGHT_REPLACEMENTS = {
    selection = "Interface\\AddOns\\LilyUI\\Media\\uf_selected.tga",
    aggro = "Interface\\AddOns\\LilyUI\\Media\\uf_aggro.tga",
    mouseover = "Interface\\AddOns\\LilyUI\\Media\\uf_mouseover.tga",
}
...
local function EnsureBaseAnchor(frame)
    if not frame or frame.__lilyuiBaseAnchor then return end
    local point, relativeTo, relativePoint, x, y = frame:GetPoint(1)
    if not point then return end
    frame.__lilyuiBaseAnchor = {
        point = point,
...
function Engine:QueuePendingResize(frame, width, height)
    ...
    frame.__lilyuiPendingResize = frame.__lilyuiPendingResize or {}
    frame.__lilyuiPendingResize.width = width
    frame.__lilyuiPendingResize.height = height
...
function Engine:UpdatePartySpacing(cfg)
    ...
    local firstAnchor = firstFrame and firstFrame.__lilyuiBaseAnchor
    local prevVisible
    ...
    for i = 1, 5 do
        ...
        if frame then
            ...
            local base = frame.__lilyuiBaseAnchor
            if base then
...
local CUSTOM_DEBUFF_BORDER = "Interface\\AddOns\\LilyUI\\Media\\white_border.tga"
...
local function ApplyPartyPlayerHiddenState(frame, cfg, hide)
    ...
    if not frame.__lilyuiPartyPlayerHideHooked then
        frame.__lilyuiPartyPlayerHideHooked = true
        frame:HookScript("OnShow", function(self)
            if self.__lilyuiPartyPlayerHidden then
                self:SetAlpha(0)
    ...
    if hide then
        if not frame.__lilyuiPartyPlayerHidden then
            frame.__lilyuiPartyPlayerHidden = true
            if frame.IsMouseEnabled and frame:IsMouseEnabled() then
                frame.__lilyuiPartyPlayerMouseWasEnabled = true
                if frame.EnableMouse then
                    frame:EnableMouse(false)
                end
            end
            frame.__lilyuiPartyPlayerWasShown = frame:IsShown()
        end
        frame:SetAlpha(0)
        frame:Hide()
        return
    end
    if frame.__lilyuiPartyPlayerHidden then
        frame.__lilyuiPartyPlayerHidden = nil
        if frame.__lilyuiPartyPlayerMouseWasEnabled and frame.EnableMouse then
            frame:EnableMouse(true)
        end
        frame.__lilyuiPartyPlayerMouseWasEnabled = nil
        local opacity = cfg and cfg.layout and cfg.layout.opacity or 1
        frame:SetAlpha(opacity)
        if frame.__lilyuiPartyPlayerWasShown ~= false then
            frame:Show()
        end
        frame.__lilyuiPartyPlayerWasShown = nil
    end
end
...
local function AcquireDebuffHighlightTexture(frame)
    if not frame then return nil end
    if frame.__lilyuiDebuffHighlight then
        return frame.__lilyuiDebuffHighlight
    end
    ...
    frame.__lilyuiDebuffHighlight = highlight
    return highlight
end
...
local function UpdateMouseoverState(frame, entering)
    if not frame then return end
    frame.__lilyuiMouseoverActive = entering == true
    local mode = DetermineMode(frame)
...
function Engine:GetConfig(mode)
    if not LilyUI.db or not LilyUI.db.profile then return nil end
    if mode == "raid" then
        return LilyUI.db.profile.raidFrames
    elseif mode == "party" then
        return LilyUI.db.profile.partyFrames
    end
    return nil
end
...
function Engine:EnsureOverlay(frame)
    if frame.LilyUIOverlay then return frame.LilyUIOverlay end
    local overlay = CreateFrame("Frame", nil, frame)
    overlay:SetAllPoints(frame)
    overlay:SetFrameLevel(frame:GetFrameLevel() + 70)
    frame.LilyUIOverlay = overlay
    return overlay
end

function Engine:EnsureBorder(frame)
    if frame.LilyUIBorder then return frame.LilyUIBorder end
    local border = CreateFrame("Frame", nil, frame, "BackdropTemplate")
    border:SetFrameLevel(frame:GetFrameLevel() + 55)
    frame.LilyUIBorder = border
    return border
end
...
local function EnsureAuraBorder(frame, size)
    ...
    size = size or 1
    if size <= 0 then
        if frame.__lilyuiAuraBorder then
            frame.__lilyuiAuraBorder:Hide()
        end
        return nil
    end
    ...
    local border = frame.__lilyuiAuraBorder
    if not border then
        border = CreateFrame("Frame", nil, frame, "BackdropTemplate")
        frame.__lilyuiAuraBorder = border
    ...
    if not border.__lilyuiEdgeSize or border.__lilyuiEdgeSize ~= size then
        border:SetBackdrop({
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = size,
            insets = { left = 0, right = 0, top = 0, bottom = 0 },
        })
        border.__lilyuiEdgeSize = size
    end
    border:SetBackdropBorderColor(0, 0, 0, 1)
    border:Show()
    return border
...
function Engine:ApplyLayout(frame, cfg)
    ...
    if layout.useCustomSize then
        if not frame.__lilyuiOriginal then
            frame.__lilyuiOriginal = {width = currentWidth, height = currentHeight}
        end
    end
    ...
    elseif frame.__lilyuiOriginal then
        targetWidth = frame.__lilyuiOriginal.width or currentWidth
        targetHeight = frame.__lilyuiOriginal.height or currentHeight
    end
    ...
    if borderSize > 0 then
        ...
        border:Show()
    elseif frame.LilyUIBorder then
        frame.LilyUIBorder:Hide()
    end
end
...
function Engine:ApplyHealth(frame, cfg)
    ...
    frame.healthBarBG:SetStatusBarTexture(LilyUI:GetTexture() or "Interface\\TargetingFrame\\UI-StatusBar")
...
function Engine:ApplyTexts(frame, cfg)
    ...
    if healthCfg and healthCfg.enabled then
        if not frame.LilyUIHealthText then
            frame.LilyUIHealthText = overlay:CreateFontString(nil, "OVERLAY")
            frame.LilyUIHealthText:SetDrawLayer("OVERLAY", 3)
        end
        if frame.LilyUIHealthText.SetDrawLayer then
            frame.LilyUIHealthText:SetDrawLayer("OVERLAY", 3)
        end
        local font = ResolveFont(healthCfg.font)
        frame.LilyUIHealthText:SetFont(font, healthCfg.size or 12, healthCfg.outline or "OUTLINE")
        frame.LilyUIHealthText:ClearAllPoints()
        frame.LilyUIHealthText:SetPoint(healthCfg.anchor or "CENTER", frame, healthCfg.anchor or "CENTER", healthCfg.offsetX or 0, healthCfg.offsetY or 0)
        frame.LilyUIHealthText:SetText(self:FormatHealth(frame, cfg))
        frame.LilyUIHealthText:SetShadowOffset(healthCfg.shadowOffsetX or 0, healthCfg.shadowOffsetY or 0)
        if not healthCfg.useClassColor then
            local color = healthCfg.color or {1, 1, 1, 1}
            frame.LilyUIHealthText:SetTextColor(color[1], color[2], color[3], color[4] or 1)
        else
            local unit = GetUnitToken(frame)
            local cr, cg, cb = GetUnitClassColor(unit)
            if cr then
                frame.LilyUIHealthText:SetTextColor(cr, cg, cb)
            end
        end
        frame.LilyUIHealthText:Show()
    elseif frame.LilyUIHealthText then
        frame.LilyUIHealthText:Hide()
    end

    local nameCfg = cfg.text and cfg.text.name
    if nameCfg and nameCfg.enabled then
        if not frame.LilyUINameText then
            frame.LilyUINameText = overlay:CreateFontString(nil, "OVERLAY")
            frame.LilyUINameText:SetDrawLayer("OVERLAY", 4)
        end
        if frame.LilyUINameText.SetDrawLayer then
            frame.LilyUINameText:SetDrawLayer("OVERLAY", 4)
        end
        local target = frame.LilyUINameText
        ...
        if frame.LilyUINameText then
            frame.LilyUINameText:Hide()
        end
        if frame.name then
            frame.name:Show()
        end
...
local function HandleStandardIcon(frame, iconKey, settings, refreshFunc)
    ...
    if settings.enabled then
        icon.__lilyuiForcedHidden = true
        if icon.Hide then
            icon:Hide()
        end
        return
    end
    if icon.__lilyuiForcedHidden then
        icon.__lilyuiForcedHidden = nil
        if refreshFunc then
            refreshFunc(frame)
        end
    end
...
local function EnsureDispelIconLayout(icon)
    if not icon or icon.__lilyuiDispelLayout then return end
    local point, relativeTo, relativePoint, offsetX, offsetY = icon:GetPoint(1)
    icon.__lilyuiDispelLayout = {
...
local function ApplyDispelIconLayout(icon, settings, index)
    ...
    local layout = icon.__lilyuiDispelLayout
    ...
    for i = 1, 3 do
        ...
        if icon then
            if settings.hide then
                if not icon.__lilyuiDispelHidden then
                    icon.__lilyuiDispelHidden = true
                    if icon.Hide then
                        icon:Hide()
                    end
                end
            else
                if icon.__lilyuiDispelHidden then
                    icon.__lilyuiDispelHidden = nil
                    if icon.Show then
                        icon:Show()
                    end
                end
...
function Engine:ApplyIcons(frame, cfg)
    ...
    if frame.centerStatusIcon and centerSettings then
        ...
        if centerSettings.enabled then
            frame.centerStatusIcon.__lilyuiForcedHidden = true
            ...
            if frame.centerStatusIcon.__lilyuiForcedHidden then
                frame.centerStatusIcon.__lilyuiForcedHidden = nil
...
function Engine:UpdateResource(frame, cfg)
    if not frame or not cfg or not cfg.resource then
        if frame and frame.LilyUIResourceBar then
            frame.LilyUIResourceBar:Hide()
        end
        return
    end
    local settings = cfg.resource
    if settings.enabled == false then
        if frame.LilyUIResourceBar then
            frame.LilyUIResourceBar:Hide()
        end
        return
    end
    if not frame.LilyUIResourceBar then
        frame.LilyUIResourceBar = CreateFrame("StatusBar", nil, frame)
        frame.LilyUIResourceBar.bg = frame.LilyUIResourceBar:CreateTexture(nil, "BACKGROUND")
        frame.LilyUIResourceBar.bg:SetAllPoints()
        frame.LilyUIResourceBar.border = CreateFrame("Frame", nil, frame.LilyUIResourceBar, "BackdropTemplate")
        frame.LilyUIResourceBar.border:SetFrameLevel(frame.LilyUIResourceBar:GetFrameLevel() + 1)
        frame.LilyUIResourceBar.border:SetBackdrop({
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
            insets = {left = 0, right = 0, top = 0, bottom = 0},
        })
        frame.LilyUIResourceBar.border:SetBackdropBorderColor(0, 0, 0, 1)
    end
    local bar = frame.LilyUIResourceBar
...
function Engine:UpdateAbsorbs(frame, cfg)
    ...
    UpdateAbsorbLayer(frame, unit, healCfg, "LilyUIHealAbsorb", UnitGetTotalHealAbsorbs, healDefaultColor)
    UpdateAbsorbLayer(frame, unit, damageCfg, "LilyUIDamageAbsorb", UnitGetTotalAbsorbs, damageDefaultColor)
end
...
local function ApplyBlizzardHighlightTexture(texture, options, replacement)
    ...
    texture.__lilyuiHighlightOriginal = texture.__lilyuiHighlightOriginal or {
    ...
    local original = texture.__lilyuiHighlightOriginal
...
function Engine:ApplyMouseoverHighlight(frame, cfg)
    ...
    if not settings or settings.enabled == false then
        if frame.LilyUIMouseoverHighlight then
            frame.LilyUIMouseoverHighlight:Hide()
        end
        return
    end
    if not frame.LilyUIMouseoverHighlight then
        frame.LilyUIMouseoverHighlight = frame:CreateTexture(nil, "OVERLAY")
        frame.LilyUIMouseoverHighlight:SetAllPoints(frame)
        frame.LilyUIMouseoverHighlight:SetTexture(HIGHLIGHT_REPLACEMENTS.mouseover)
        frame.LilyUIMouseoverHighlight:SetDrawLayer("OVERLAY", 2)
    end
    frame.LilyUIMouseoverHighlight:SetAlpha(settings.alpha or 0.5)
    frame.LilyUIMouseoverHighlight:SetShown(frame.__lilyuiMouseoverActive == true)
end
...
function Engine:UpdateHighlights(frame, cfg)
    ...
    if selection and selection.mode ~= "BLIZZARD" then
        if not frame.LilyUISelectionHighlight then
            frame.LilyUISelectionHighlight = frame:CreateTexture(nil, "OVERLAY")
            frame.LilyUISelectionHighlight:SetAllPoints(frame)
        end
        local color = selection.color or {1, 1, 1, 1}
        local selectionAlpha = selection.alpha
        frame.LilyUISelectionHighlight:SetColorTexture(color[1], color[2], color[3], 1)
        frame.LilyUISelectionHighlight:SetAlpha(selectionAlpha or color[4] or 0.35)
        local unit = GetUnitToken(frame)
        ...
        frame.LilyUISelectionHighlight:SetShown(showSelection)
    elseif frame.LilyUISelectionHighlight then
        frame.LilyUISelectionHighlight:Hide()
    end

    local aggro = cfg.highlights.aggro
    if aggro and aggro.mode ~= "BLIZZARD" then
        if not frame.LilyUIAggroHighlight then
            local overlay = self:EnsureOverlay(frame)
            frame.LilyUIAggroHighlight = overlay:CreateTexture(nil, "OVERLAY")
            frame.LilyUIAggroHighlight:SetAllPoints(frame)
        end
        if frame.LilyUIAggroHighlight.SetDrawLayer then
            frame.LilyUIAggroHighlight:SetDrawLayer("OVERLAY", 7)
        end
        local unit = GetUnitToken(frame)
        local status = unit and UnitThreatSituation("player", unit)
        if status and status > 0 then
            local r, g, b = GetThreatStatusColor(status)
            frame.LilyUIAggroHighlight:SetColorTexture(r, g, b, 0.35)
            frame.LilyUIAggroHighlight:Show()
        else
            frame.LilyUIAggroHighlight:Hide()
        end
    elseif frame.LilyUIAggroHighlight then
        frame.LilyUIAggroHighlight:Hide()
    end
...
function Engine:ApplyFrameSpacing(frame, cfg, mode)
    if mode ~= "party" then
        if not UnitAffectingCombat("player") then
            EnsureBaseAnchor(frame)
            if frame.__lilyuiBaseAnchor and frame.__lilyuiBaseAnchor.active then
                local base = frame.__lilyuiBaseAnchor
                frame:ClearAllPoints()
                frame:SetPoint(base.point, base.relativeTo, base.relativePoint, base.x, base.y)
                frame.__lilyuiBaseAnchor.active = false
            end
        end
        return
    end
...
function Engine:EnsureMouseoverHandlers(frame)
    if not frame or frame.__lilyuiMouseoverHooked then return end
    frame.__lilyuiMouseoverHooked = true
    frame.__lilyuiMouseoverActive = frame:IsMouseOver() == true
...
function PartyFrames:ApplySoloVisibility()
    local cfg = LilyUI.db.profile.partyFrames
