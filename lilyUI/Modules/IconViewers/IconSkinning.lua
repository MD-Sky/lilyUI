...
if not IconViewers then
    error("lilyUI: IconViewers module not initialized! Load IconViewers.lua first.")
end
...
function IconViewers:SkinIcon(icon, settings)
	...
	if LilyUI and LilyUI.ScaleBorder then
		edgeSize = LilyUI:ScaleBorder(edgeSize)
	elseif LilyUI and LilyUI.Scale then
		edgeSize = LilyUI:Scale(edgeSize)
    ...
    local function SafeSetBackdrop(frame, backdropInfo)
        ...
        if InCombatLockdown() then
            ...
            local alreadyPending = LilyUI.__cdmPendingBackdrops and LilyUI.__cdmPendingBackdrops[frame]
            if not alreadyPending then
                ...
                if not LilyUI.__cdmBackdropEventFrame then
                    ...
                    eventFrame:SetScript("OnEvent", function(self)
                        ...
                        for pendingFrame in pairs(LilyUI.__cdmPendingBackdrops or {}) do
                ...
                LilyUI.__cdmPendingBackdrops = LilyUI.__cdmPendingBackdrops or {}
                LilyUI.__cdmPendingBackdrops[frame] = true
                LilyUI.__cdmBackdropEventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
            end
            return false
        end
    ...
    for _, icon in ipairs(children) do
        if IsCooldownIconFrame(icon) and (icon.icon or icon.Icon) then
            ...
            if not ok then
                ...
                print("|cffff4444[LilyUI] SkinIcon error for", name, "icon:", err, "|r")
            end
        end
    end
end
...
LilyUI.SkinIcon = function(self, icon, settings) return IconViewers:SkinIcon(icon, settings) end
LilyUI.SkinAllIconsInViewer = function(self, viewer) return IconViewers:SkinAllIconsInViewer(viewer) end

-- Hook to update proc glow when icons are skinned (aspect ratio changes)
if LilyUI.ProcGlow and LilyUI.ProcGlow.UpdateButtonGlow then
    ...
    function IconViewers:SkinIcon(icon, settings)
        ...
        if icon and icon.SpellActivationAlert then
            C_Timer.After(0.01, function()
                if LilyUI.ProcGlow and LilyUI.ProcGlow.UpdateButtonGlow then
                    LilyUI.ProcGlow:UpdateButtonGlow(icon)
                end
            end)
        end
