...
local function GetSettings()
    ...
    LilyUI.db.profile.buffBarViewer = LilyUI.db.profile.buffBarViewer or {}
    LilyUI.db.profile.buffBarViewer.barColors = LilyUI.db.profile.buffBarViewer.barColors or {}
    LilyUI.db.profile.buffBarViewer.barColorsBySpec = LilyUI.db.profile.buffBarViewer.barColorsBySpec or {}
    return LilyUI.db.profile.buffBarViewer
end
...
local function ComputeBarWidth(settings, viewer, iconTotal, spacing, barBorder)
    ...
    if width <= 0 then
        ...
        width = PixelSnap(LilyUI:Scale(width))
    end
...
local function ComputeBarHeight(settings, bar)
    ...
    local scaled = LilyUI:Scale(desired)
...
local function ApplyIconBorder(iconFrame, settings)
    ...
    local borderSize = LilyUI:ScaleBorder(size)
...
local function StyleBarChild(child, settings, viewer)
    ...
    local font = (LilyUI.GetGlobalFont and LilyUI:GetGlobalFont()) or nil
    local iconBorderSize = settings.iconBorderSize or 0
    local iconBorderScaled = LilyUI:ScaleBorder(iconBorderSize)
    ...
    local barBorderSize = LilyUI:ScaleBorder(settings.borderSize or 1)

    local barWidth = ComputeBarWidth(settings, viewer, iconTotalWidth, 0, 0)
    -- Bar visuals
    local tex = LilyUI.GetTexture and LilyUI:GetTexture(settings.texture) or WHITE8
    ...
    local borderSize = LilyUI:ScaleBorder(settings.borderSize or 1)
    border:ClearAllPoints()
...
LilyUI.RefreshBuffBarCooldownViewer = function(self)
