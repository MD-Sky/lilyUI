...
LilyUI.Keybinds = LilyUI.Keybinds or {}
local Keybinds = LilyUI.Keybinds
...
local function IsKeybindEnabledForAnyViewer()
    ...
    for _, viewerSettingName in pairs(viewersSettingKey) do
        ...
        if LilyUI.db.profile[enabledKey] then
            return true
        end
    end
    return false
...
local function GetKeybindSettings(viewerSettingName)
    ...
    local fontName = LilyUI.db.profile["cooldownManager_keybindFontName_" .. viewerSettingName]
    if not fontName or fontName == "" then
        fontName = LilyUI.db.profile.cooldownManager_keybindFontName or defaults.fontName
    end

    local fontColor = LilyUI.db.profile["cooldownManager_keybindFontColor_" .. viewerSettingName]
    if not fontColor then
        fontColor = LilyUI.db.profile.cooldownManager_keybindFontColor or defaults.fontColor
    end

    return {
        anchor = LilyUI.db.profile["cooldownManager_keybindAnchor_" .. viewerSettingName] or defaults.anchor,
        fontSize = LilyUI.db.profile["cooldownManager_keybindFontSize_" .. viewerSettingName] or defaults.fontSize,
        offsetX = LilyUI.db.profile["cooldownManager_keybindOffsetX_" .. viewerSettingName] or defaults.offsetX,
        offsetY = LilyUI.db.profile["cooldownManager_keybindOffsetY_" .. viewerSettingName] or defaults.offsetY,
        fontColor = fontColor,
        fontName = fontName,
    }
end
...
local function ApplyKeybindTextSettings(icon, viewerSettingName)
    ...
    local fontFlags = LilyUI.db.profile.cooldownManager_keybindFontFlags or {}
...
local function InjectCachedDataOntoIcons()
    ...
    PrintDebug("[LilyUI Keybinds] Injected cached data onto", injectedCount, "icons")
...
local function UpdateIconKeybind(icon, viewerSettingName)
    ...
    local enabledKey = "cooldownManager_showKeybinds_" .. viewerSettingName
    if not LilyUI.db.profile[enabledKey] then
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "EDIT_MODE_LAYOUTS_UPDATED" then
        PrintDebug("[LilyUI Keybinds] EditMode layout changed - rebuilding cache")
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        Keybinds:UpdateAllKeybinds()
        PrintDebug(
            "[LilyUI Keybinds] PLAYER_ENTERING_WORLD - LoadOrBuild result:",
            "inLockdown:",
            tostring(InCombatLockdown())
        )
    elseif
...
function Keybinds:Shutdown()
    PrintDebug("[LilyUI Keybinds] Shutting down module")
    ...
    if LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.keybindCache then
        LilyUI.db.profile.keybindCache = nil

        PrintDebug("[LilyUI Keybinds] Wiped DB cache")
    end
...
function Keybinds:Enable()
    ...
    PrintDebug("[LilyUI Keybinds] Enabling module")
        ...
        for viewerName, settingName in pairs(viewersSettingKey) do
            ...
            if viewerFrame then
                hooksecurefunc(viewerFrame, "RefreshLayout", function()
                    ...
                    PrintDebug("[LilyUI Keybinds] RefreshLayout called for viewer:", viewerName)
...
function Keybinds:Disable()
    ...
    PrintDebug("[LilyUI Keybinds] Disabling module")
...
function Keybinds:Initialize()
    if not IsKeybindEnabledForAnyViewer() then
        PrintDebug("[LilyUI Keybinds] Not initializing - no viewers enabled")
        return
    end

    PrintDebug("[LilyUI Keybinds] Initializing module")
