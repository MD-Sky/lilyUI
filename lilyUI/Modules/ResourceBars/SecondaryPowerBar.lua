...
local ResourceBars = LilyUI.ResourceBars
if not ResourceBars then
    error("lilyUI: ResourceBars module not initialized! Load ResourceDetection.lua first.")
end
...
function ResourceBars:GetSecondaryPowerBar()
    if LilyUI.secondaryPowerBar then return LilyUI.secondaryPowerBar end

    local cfg = LilyUI.db.profile.secondaryPowerBar
    ...
    bar:SetHeight(LilyUI:Scale(cfg.height or 4))
    bar:SetPoint("CENTER", anchor, anchorPoint, LilyUI:Scale(cfg.offsetX or 0), LilyUI:Scale(cfg.offsetY or 12))
    ...
    local borderSize = LilyUI:ScaleBorder(cfg.borderSize or 1)
    ...
    bar.TextValue:SetPoint("CENTER", bar.TextFrame, "CENTER", LilyUI:Scale(cfg.textX or 0), LilyUI:Scale(cfg.textY or 0))
    bar.TextValue:SetJustifyH("CENTER")
    bar.TextValue:SetFont(LilyUI:GetGlobalFont(), cfg.textSize or 12, "OUTLINE")
...
function ResourceBars:UpdateSecondaryPowerBarTicks(bar, resource, max)
    ...
    for i = 1, needed do
        ...
        local tickWidth = math.max(1, LilyUI:Scale(1))
...
function ResourceBars:UpdateSecondaryPowerBar()
    ...
    if not cfg.enabled then
        if LilyUI.secondaryPowerBar then
            LilyUI.secondaryPowerBar:Hide()
            LilyUI.secondaryPowerBar:SetScript("OnUpdate", nil)
        end
        return
    end
    ...
    if not anchor or not anchor:IsShown() then
        if LilyUI.secondaryPowerBar then LilyUI.secondaryPowerBar:Hide() end
        return
    end
    ...
    -- Update layout
    local anchorPoint = cfg.anchorPoint or "CENTER"
    local desiredHeight = LilyUI:Scale(cfg.height or 4)
    local desiredX = LilyUI:Scale(cfg.offsetX or 0)
    local desiredY = LilyUI:Scale(cfg.offsetY or 12)

    local width = cfg.width or 0
    if width <= 0 then
        width = PixelSnap(
            anchor.__cdmIconWidth
            or (LilyUI.powerBar and LilyUI.powerBar:IsShown() and LilyUI.powerBar:GetWidth())
            or anchor:GetWidth()
        )
        -- Width is already in pixels, no need to scale again
    else
    ...
    if bar.Border then
        local scaledBorder = LilyUI:ScaleBorder(borderSize)
    ...
    if fragmentedPowerTypes[resource] then
        ...
        local powerTypeColors = LilyUI.db.profile.powerTypeColors
    ...
    if cfg.hideBarShowText then
        ...
        if bar.Border and (bar._scaledBorder or LilyUI:ScaleBorder(cfg.borderSize or 1)) > 0 then
            bar.Border:Show()
        end
...
LilyUI.GetSecondaryPowerBar = function(self) return ResourceBars:GetSecondaryPowerBar() end
LilyUI.UpdateSecondaryPowerBar = function(self) return ResourceBars:UpdateSecondaryPowerBar() end
LilyUI.UpdateSecondaryPowerBarTicks = function(self, bar, resource, max) return ResourceBars:UpdateSecondaryPowerBarTicks(bar, resource, max) end
LilyUI.CreateFragmentedPowerBars = function(self, bar, resource) return ResourceBars:CreateFragmentedPowerBars(bar, resource) end
LilyUI.UpdateFragmentedPowerDisplay = function(self, bar, resource) return ResourceBars:UpdateFragmentedPowerDisplay(bar, resource) end
LilyUI.UpdateChargedPowerSegments = function(self, bar, resource, max) return ResourceBars:UpdateChargedPowerSegments(bar, resource, max) end
