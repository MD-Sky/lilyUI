...
-- Unit Frame System
LilyUI.UnitFrames = LilyUI.UnitFrames or {}
local UF = LilyUI.UnitFrames

-- Unit to frame name mapping
local UnitToFrameName = {
    player = "LilyUI_Player",
    target = "LilyUI_Target",
    targettarget = "LilyUI_TargetTarget",
    pet = "LilyUI_Pet",
    focus = "LilyUI_Focus",
    boss = "LilyUI_Boss",
}
...
local function EnsureDelayedInitListener()
    ...
    UF._delayedInitFrame:SetScript("OnEvent", function(self, event)
        if event == "PLAYER_REGEN_ENABLED" then
            ...
            if LilyUI.UnitFrames and LilyUI.UnitFrames.Initialize then
...
function UF.ResolveFrameName(unit)
    if unit == "player" then
        return "LilyUI_Player"
    elseif unit == "target" then
        return "LilyUI_Target"
    elseif unit == "focus" then
        return "LilyUI_Focus"
    elseif unit:match("^boss%d+$") then
        return "LilyUI_" .. unit:gsub("^b", "B")
    elseif unit == "targettarget" then
        return "LilyUI_TargetTarget"
    elseif unit == "focustarget" then
        return "LilyUI_FocusTarget"
...
function UF:ResolveMedia()
    ...
    self.Media.Font = LilyUI:GetGlobalFont()

    -- Check for texture overrides in General settings, otherwise use global texture
    local db = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.unitFrames
    ...
    self.Media.ForegroundTexture = LilyUI:GetTexture(foregroundOverride)
    self.Media.BackgroundTexture = LilyUI:GetTexture(backgroundOverride)
end
...
function UF:ApplyBossPreviewData(unitFrame, bossIndex)
    ...
    local bossDB = LilyUI.db.profile.unitFrames.boss
    if not bossDB then return end
    local frameDB = bossDB.Frame or {}
    local powerBarDB = bossDB.PowerBar or {}
    local generalDB = (LilyUI.db.profile.unitFrames and LilyUI.db.profile.unitFrames.General) or {}
...
function UF:ShowBossFramesPreview()
    if not LilyUI.db.profile.unitFrames or not LilyUI.db.profile.unitFrames.enabled then return end
...
function UF:CreateBossAnchor()
    ...
    local anchor = CreateFrame("Frame", "LilyUI_BossAnchor", UIParent)
...
function UF:UpdateBossAnchor()
    ...
    if showAnchor then
        ...
        local boss1Frame = _G["LilyUI_Boss1"]
...
function UF:HideBossFramesPreview()
    ...
    for i = 1, 8 do
        ...
        if LilyUI.bossCastBars and LilyUI.bossCastBars[i] then
            LilyUI.bossCastBars[i]:Hide()
        end
    end

    -- Hide associated boss cast bars
    if LilyUI.CastBars and LilyUI.CastBars.HideTestBossCastBars then
        LilyUI.CastBars:HideTestBossCastBars()
    end
end
...
function UF:RefreshFrames()
    ...
    for _, unit in ipairs({"player", "target", "focus"}) do
        if LilyUI.db.profile.unitFrames[unit] and LilyUI.db.profile.unitFrames[unit].Enabled then
            self:UpdateUnitFrame(unit)
        end
    end

    -- Refresh boss frames
    if LilyUI.db.profile.unitFrames.boss and LilyUI.db.profile.unitFrames.boss.Enabled then
        for i = 1, 8 do
            self:UpdateUnitFrame("boss" .. i)
        end
        self:LayoutBossFrames()
