...
LilyUI.bossCastBars = LilyUI.bossCastBars or {}

local function SetBossCastBarColor(bossIndex, state)
    local cfg = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.bossCastBar
    if not cfg then return end

    local bar = LilyUI.bossCastBars[bossIndex]
...
function CastBars:GetBossCastBar(bossIndex)
    if LilyUI.bossCastBars[bossIndex] then return LilyUI.bossCastBars[bossIndex] end

    local cfg = LilyUI.db.profile.bossCastBar
    local frameName = "LilyUI_Boss" .. bossIndex
    ...
    bar:SetPoint(anchorPoint, anchor, anchorPoint, LilyUI:Scale(cfg.offsetX or 0), LilyUI:Scale(cfg.offsetY or 0))
    ...
    local tex = LilyUI:GetTexture(LilyUI.db.profile.bossCastBar.texture)
    ...
    LilyUI.bossCastBars[bossIndex] = bar
    return bar
end
        ...
        if bossFrame and bossFrame.spellbar and not bossFrame.spellbar.__lilyuiHooked then
            ...
            bossFrame.spellbar:HookScript("OnShow", function(self)
                ...
                if not cfg or not cfg.enabled then
                    if LilyUI.bossCastBars[i] then LilyUI.bossCastBars[i]:Hide() end
                    return
                end
            ...
            bossFrame.spellbar:HookScript("OnHide", function()
                if LilyUI.bossCastBars[i] then
                    LilyUI.bossCastBars[i]:Hide()
                end
            end)
            ...
            bossFrame.spellbar:HookScript("OnUpdate", function(self, elapsed)
                ...
                local bar = LilyUI.bossCastBars[i]
...
function CastBars:ShowTestBossCastBars()
    ...
    if not LilyUI.UnitFrames or not LilyUI.UnitFrames.BossPreviewMode then
        print("Boss frames must be in preview mode to test cast bars")
        return
    end

    for i = 1, 8 do
        local unitFrame = _G["LilyUI_Boss" .. i]
...
LilyUI.GetBossCastBar = function(self, bossIndex) return CastBars:GetBossCastBar(bossIndex) end
LilyUI.UpdateBossCastBarLayout = function(self, bossIndex) return CastBars:UpdateBossCastBarLayout(bossIndex) end
LilyUI.UpdateAllBossCastBarLayouts = function(self) return CastBars:UpdateAllBossCastBarLayouts() end
LilyUI.HookBossCastBars = function(self) return CastBars:HookBossCastBars() end
LilyUI.ShowTestBossCastBars = function(self) return CastBars:ShowTestBossCastBars() end
