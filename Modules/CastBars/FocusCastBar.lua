...
local function SetFocusCastBarColor(state)
    local cfg = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.focusCastBar
    if not cfg then return end

    local bar = LilyUI.focusCastBar
...
function CastBars:GetFocusCastBar()
    if LilyUI.focusCastBar then return LilyUI.focusCastBar end
    ...
    bar:SetPoint("CENTER", anchor, anchorPoint, LilyUI:Scale(cfg.offsetX or 0), LilyUI:Scale(cfg.offsetY or -50))
    ...
    local tex = LilyUI:GetTexture(LilyUI.db.profile.focusCastBar.texture)
...
function CastBars:HookFocusCastBar()
    ...
    if focusSpellbar and not focusSpellbar.__lilyuiHooked then
        focusSpellbar.__lilyuiHooked = true
        
        focusSpellbar:HookScript("OnShow", function(self)
            local cfg = LilyUI.db.profile.focusCastBar
            if not cfg or not cfg.enabled then
                if LilyUI.focusCastBar then LilyUI.focusCastBar:Hide() end
                return
            end
        ...
        focusSpellbar:HookScript("OnHide", function()
            if LilyUI.focusCastBar then
                LilyUI.focusCastBar:Hide()
            end
        end)
...
LilyUI.GetFocusCastBar = function(self) return CastBars:GetFocusCastBar() end
LilyUI.UpdateFocusCastBarLayout = function(self) return CastBars:UpdateFocusCastBarLayout() end
