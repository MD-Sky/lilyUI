...
local function StartRuneUpdateTicker()
    ...
    runeUpdateTicker = C_Timer.NewTicker(0.1, function()
        local cfg = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.secondaryPowerBar
...
function ResourceBars:Initialize()
    ...
    LilyUI:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED", function()
        ResourceBars:OnSpecChanged()
    end)
    LilyUI:RegisterEvent("UPDATE_SHAPESHIFT_FORM", function()
        ResourceBars:OnShapeshiftChanged()
    end)
    LilyUI:RegisterEvent("PLAYER_ENTERING_WORLD", function()
        ResourceBars:OnUnitPower()
    end)

    -- POWER UPDATES
    LilyUI:RegisterEvent("UNIT_POWER_FREQUENT", function(_, unit)
        ResourceBars:OnUnitPower(_, unit)
    end)
    LilyUI:RegisterEvent("UNIT_POWER_UPDATE", function(_, unit)
        ResourceBars:OnUnitPower(_, unit)
    end)
    LilyUI:RegisterEvent("UNIT_MAXPOWER", function(_, unit)
        ResourceBars:OnUnitPower(_, unit)
    end)
    ...
    LilyUI:RegisterEvent("RUNE_POWER_UPDATE", function()
        ResourceBars:OnRuneEvent()
    end)
    LilyUI:RegisterEvent("RUNE_TYPE_UPDATE", function()
        ResourceBars:OnRuneEvent()
    end)
...
LilyUI.OnUnitPower = function(self, _, unit) return ResourceBars:OnUnitPower(_, unit) end
LilyUI.OnSpecChanged = function(self) return ResourceBars:OnSpecChanged() end
LilyUI.OnShapeshiftChanged = function(self) return ResourceBars:OnShapeshiftChanged() end
