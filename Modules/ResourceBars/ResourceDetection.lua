...
LilyUI.ResourceBars = LilyUI.ResourceBars or {}
LilyUI.ResourceBars.tickedPowerTypes = tickedPowerTypes
LilyUI.ResourceBars.fragmentedPowerTypes = fragmentedPowerTypes
LilyUI.ResourceBars.HAS_UNIT_POWER_PERCENT = HAS_UNIT_POWER_PERCENT
LilyUI.ResourceBars.buildVersion = buildVersion
...
local function GetResourceColor(resource)
    ...
    if resource == "STAGGER" then
        ...
        if percent >= 60 then
            ...
            local heavyColor = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.powerTypeColors and LilyUI.db.profile.powerTypeColors.colors and LilyUI.db.profile.powerTypeColors.colors["STAGGER_HEAVY"]
            color = heavyColor or { r = 1.00, g = 0.42, b = 0.42 }
        elseif percent >= 30 then
            -- Medium stagger - use configured medium color or default yellow
            local mediumColor = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.powerTypeColors and LilyUI.db.profile.powerTypeColors.colors and LilyUI.db.profile.powerTypeColors.colors["STAGGER_MEDIUM"]
            color = mediumColor or { r = 1.00, g = 0.98, b = 0.72 }
        else
            -- Light stagger - use configured light color or default green
            local lightColor = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.powerTypeColors and LilyUI.db.profile.powerTypeColors.colors and LilyUI.db.profile.powerTypeColors.colors["STAGGER_LIGHT"]
...
local function GetSecondaryResourceValue(resource, cfg)
    ...
    cfg = cfg or (LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.secondaryPowerBar) or {}
...
-- Export functions
LilyUI.ResourceBars.GetPrimaryResource = GetPrimaryResource
LilyUI.ResourceBars.GetSecondaryResource = GetSecondaryResource
LilyUI.ResourceBars.GetResourceColor = GetResourceColor
LilyUI.ResourceBars.GetPrimaryResourceValue = GetPrimaryResourceValue
LilyUI.ResourceBars.GetSecondaryResourceValue = GetSecondaryResourceValue
LilyUI.ResourceBars.GetChargedPowerPoints = GetChargedPowerPoints
