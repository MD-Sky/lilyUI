...
function ResourceBars:GetPowerBar()
    if LilyUI.powerBar then return LilyUI.powerBar end

    local cfg = LilyUI.db.profile.powerBar
    ...
    bar:SetHeight(LilyUI:Scale(cfg.height or 6))
    bar:SetPoint("CENTER", anchor, anchorPoint, LilyUI:Scale(cfg.offsetX or 0), LilyUI:Scale(cfg.offsetY or 6))
...
function ResourceBars:UpdatePowerBar()
    ...
    if not cfg.enabled then
        if LilyUI.powerBar then
            LilyUI.powerBar:Hide()
            LilyUI.powerBar:SetScript("OnUpdate", nil)
        end
        return
    end
    ...
    -- Update layout
    local anchorPoint = cfg.anchorPoint or "CENTER"
    local desiredHeight = LilyUI:Scale(cfg.height or 6)
    local desiredX = LilyUI:Scale(cfg.offsetX or 0)
    local desiredY = LilyUI:Scale(cfg.offsetY or 6)
...
LilyUI.GetPowerBar = function(self) return ResourceBars:GetPowerBar() end
LilyUI.UpdatePowerBar = function(self) return ResourceBars:UpdatePowerBar() end
LilyUI.UpdatePowerBarTicks = function(self, bar, resource, max) return ResourceBars:UpdatePowerBarTicks(bar, resource, max) end
