...
LilyUI.AbsorbBars = LilyUI.AbsorbBars or {}
local AbsorbBars = LilyUI.AbsorbBars
...
local function CreateAbsorbBarForUnit(unit)
    ...
    if not frameName and unit:match("^boss%d+$") then
        frameName = "LilyUI_Boss" .. unit:match("^boss(%d+)$")
    end
    ...
    local function UpdateAbsorbBarTexture()
        ...
        if not db or not db[unit] or not db[unit].AbsorbBar then
            local tex = LilyUI:GetGlobalTexture()
            if tex then
        ...
        local textureName = db[unit].AbsorbBar.Texture or "Lily"
