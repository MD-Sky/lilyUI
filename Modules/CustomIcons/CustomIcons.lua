...
local Widgets = LilyUI.GUI and LilyUI.GUI.Widgets
local THEME = LilyUI.GUI and LilyUI.GUI.THEME
...
local function GetDynamicDB()
    local profile = LilyUI.db.profile
...
local function SafeSetBackdrop(frame, backdropInfo, borderColor)
    if not frame or not frame.SetBackdrop then return end
        if InCombatLockdown() then
            if not LilyUI.__cdmPendingBackdrops then
                LilyUI.__cdmPendingBackdrops = {}
            end
        LilyUI.__cdmPendingBackdrops[frame] = {backdropInfo = backdropInfo, borderColor = borderColor}
            if not LilyUI.__cdmBackdropEventFrame then
                local eventFrame = CreateFrame("Frame")
                eventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
            eventFrame:SetScript("OnEvent", function()
                for pending, settings in pairs(LilyUI.__cdmPendingBackdrops) do
                ...
                LilyUI.__cdmBackdropEventFrame = eventFrame
            end
        return
    end
...
local function ApplyCooldownTextStyle(cooldown, iconData)
    ...
    if LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.viewers and LilyUI.db.profile.viewers.general then
        shadowOffsetX = LilyUI.db.profile.viewers.general.cooldownShadowOffsetX or shadowOffsetX
        shadowOffsetY = LilyUI.db.profile.viewers.general.cooldownShadowOffsetY or shadowOffsetY
    end
...
local function ApplyIconSettings(iconFrame, iconData)
    ...
    if iconFrame.cooldown.SetCountdownFont then
        local cdFontPath = LilyUI:GetGlobalFont()
...
function CustomIcons:ShowLoadConditionsWindow(iconKey, iconData)
    ...
    local f = CreateFrame("Frame", "LilyUI_LoadConditions", UIParent, "BackdropTemplate")
    f:SetSize(360, 460)
    f:SetPoint("CENTER")
...
local function CreateItemIcon(iconKey, iconData, parent)
    ...
    local frame = CreateBaseIcon("LilyUI_DynItem_" .. iconKey, parent)
    frame._type = "item"
...
local function CreateSpellIcon(iconKey, iconData, parent)
    ...
    local frame = CreateBaseIcon("LilyUI_DynSpell_" .. iconKey, parent)
    frame._type = "spell"
...
local function CreateSlotIcon(iconKey, iconData, parent)
    ...
    local frame = CreateBaseIcon("LilyUI_DynSlot_" .. iconKey, parent)
    frame._type = "slot"
...
local function EnsureGroupFrame(groupKey, settings)
    ...
    local container = CreateFrame("Frame", "LilyUI_DynGroup_" .. groupKey, UIParent)
...
function CustomIcons:CreateCustomIconsTrackerFrame()
    if not LilyUI.db.profile.customIcons.enabled then return nil end

    -- Create the main container frame (for backwards compatibility)
    if not LilyUI.customIconsTrackerFrame then
        LilyUI.customIconsTrackerFrame = CreateFrame("Frame", "LilyUI_CustomIconsTrackerFrame", UIParent)
        LilyUI.customIconsTrackerFrame:SetSize(200, 40)
        LilyUI.customIconsTrackerFrame:SetFrameStrata("MEDIUM")
        LilyUI.customIconsTrackerFrame:SetClampedToScreen(true)
        LilyUI.customIconsTrackerFrame:SetPoint("CENTER", UIParent, "CENTER", 0, -200)
        LilyUI.customIconsTrackerFrame._LilyUI_CustomIconsTracker = true
    end

    -- Load all dynamic icons
    self:LoadDynamicIcons()

    return LilyUI.customIconsTrackerFrame
end
        ...
        do
            ...
            Widgets.CreateSelect(uiFrames.configParent, {
                ...
                get = function()
                    ...
                    return cs.font or (LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.general and LilyUI.db.profile.general.globalFont) or "Expressway"
                end,
                set = function(_, val)
                    iconData.settings.countSettings = iconData.settings.countSettings or {}
...
function CustomIcons:ConfirmDeleteIcon(iconKey, label)
    if not uiFrames.confirmFrame then
        local f = CreateFrame("Frame", "LilyUI_DynIconConfirm", UIParent, "BackdropTemplate")
        f:SetSize(320, 140)
        f:SetPoint("CENTER")
...
function CustomIcons:ShowCreateIconDialog()
    if not uiFrames.createFrame then
        local f = CreateFrame("Frame", "LilyUI_DynIconCreate", UIParent, "BackdropTemplate")
        f:SetSize(360, 220)
        f:SetPoint("CENTER")
        f:SetFrameStrata("TOOLTIP")
        ...
        -- Slot dropdown
        local dropdown = CreateFrame("Frame", "LilyUI_DynIconCreateSlotDrop", f, "UIDropDownMenuTemplate")
...
local function ShouldShowAnchors()
    ...
    local uf = LilyUI.db and LilyUI.db.profile and LilyUI.db.profile.unitFrames
...
local function EnsureAnchorHooks()
    ...
    if LilyUI.UnitFrames and LilyUI.UnitFrames.UpdateEditModeAnchors then
        hooksecurefunc(LilyUI.UnitFrames, "UpdateEditModeAnchors", function()
            CustomIcons:RefreshAnchorVisibility()
        end)
        anchorHooked = true
    end
...
if LilyUI.db and LilyUI.db.profile then
    CustomIcons:LoadDynamicIcons()
    CustomIcons:RefreshAnchorVisibility()
    EnsureAnchorHooks()
