...
LilyUI.ActionBars = LilyUI.ActionBars or {}
local ActionBars = LilyUI.ActionBars

local LSM = LibStub("LibSharedMedia-3.0")

-- Texture paths
local BACKDROP_TEXTURE = "Interface\\Buttons\\WHITE8x8"
local HIGHLIGHT_TEXTURE = [[Interface\AddOns\lilyUI\Media\white_border.tga]]
...
local function GetActionBarFont(cfg)
    ...
    return LilyUI:GetGlobalFont()
end

-- Scale helper that respects LilyUI's pixel perfect multiplier
local function ScaleOffset(value)
    if not value or value == 0 then
        return 0
    end
    return LilyUI:Scale(value)
end
...
local function StyleActionButton(button)
    ...
    local cfg = LilyUI.db.profile.actionBars
    ...
    local backdropOffset = (LilyUI.ScaleBorder and LilyUI:ScaleBorder(1)) or math.floor(LilyUI:Scale(1) + 0.5)
    ...
    -- Update highlight texture to LilyUI custom border
...
local function ApplyBarAlphaTransition(barFrame, shouldShow)
    ...
    local actionBarConfig = LilyUI.db.profile.actionBars
...
function ActionBars:Initialize()
    ...
    LilyUI:RegisterEvent("PLAYER_LOGIN", function()
        C_Timer.After(0.5, function()
            self:StyleAllBars()
        end)
    end)
