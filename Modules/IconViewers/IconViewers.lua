...
LilyUI.IconViewers = LilyUI.IconViewers or {}
local IconViewers = LilyUI.IconViewers

local viewers = LilyUI.viewers or {
...
function IconViewers:ApplyViewerSkin(viewer)
    ...
    local settings = LilyUI.db.profile.viewers[name]
    ...
    if LilyUI.ResourceBars and LilyUI.ResourceBars.UpdatePowerBar then
        LilyUI.ResourceBars:UpdatePowerBar()
    end
    if LilyUI.ResourceBars and LilyUI.ResourceBars.UpdateSecondaryPowerBar then
        LilyUI.ResourceBars:UpdateSecondaryPowerBar()
    end
    if LilyUI.CastBars and LilyUI.CastBars.UpdateCastBarLayout then
        LilyUI.CastBars:UpdateCastBarLayout()
    end
...
function IconViewers:ProcessPendingIcons()
    if not LilyUI.__cdmPendingIcons then return end
    if InCombatLockdown() then return end
    
    local processed = {}
    for icon, data in pairs(LilyUI.__cdmPendingIcons) do
    ...
    for icon in pairs(processed) do
        LilyUI.__cdmPendingIcons[icon] = nil
    end
    
    if not next(LilyUI.__cdmPendingIcons) then
        LilyUI.__cdmPendingIcons = nil
    end
end
...
function IconViewers:ForceRefreshBuffIcons()
    ...
    if viewer and viewer:IsShown() then
        ...
        print("|cff00ff00[LilyUI] Force refreshed BuffIconCooldownViewer|r")
    end
end
    ...
    for _, icon in ipairs(icons) do
        ...
        elseif not icon.__cdmSkinned then
            if not icon.__cdmSkinPending then
                ...
                if not LilyUI.__cdmPendingIcons then
                    LilyUI.__cdmPendingIcons = {}
                end
                LilyUI.__cdmPendingIcons[icon] = { icon = icon, settings = settings, viewer = viewer }
...
LilyUI.ApplyViewerSkin = function(self, viewer) return IconViewers:ApplyViewerSkin(viewer) end
LilyUI.HookViewers = function(self) return IconViewers:HookViewers() end
LilyUI.AutoLoadBuffIcons = function(self, retryCount) return IconViewers:AutoLoadBuffIcons(retryCount) end
LilyUI.ForceRefreshBuffIcons = function(self) return IconViewers:ForceRefreshBuffIcons() end
LilyUI.ProcessPendingIcons = function(self) return IconViewers:ProcessPendingIcons() end
