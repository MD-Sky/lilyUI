...
local function CreateTypeOptions(typeKey, displayName, order)
    local db = LilyUI.db.profile.buffDebuffFrames
    ...
    return {
        ...
        args = {
            ...
            enabled = {
                ...
                desc = "Apply custom LilyUI styling to " .. displayName:lower() .. " frames",
                width = "full",
                order = 2,
                get = function()
                    local db = LilyUI.db.profile.buffDebuffFrames
                ...
                set = function(_, val)
                    local db = LilyUI.db.profile.buffDebuffFrames
                    if not db then
                        LilyUI.db.profile.buffDebuffFrames = {}
                        db = LilyUI.db.profile.buffDebuffFrames
                    end
                    if not db.enabled then db.enabled = true end
                    if not db[typeKey] then db[typeKey] = {} end
                    db[typeKey].enabled = val
                    if LilyUI.BuffDebuffFrames and LilyUI.BuffDebuffFrames.RefreshAll then
                        LilyUI.BuffDebuffFrames:RefreshAll()
                    end
                end,
            },
            spacer0 = {
