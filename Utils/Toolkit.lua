...
if not LilyUI then
	error("LilyUI not found! Toolkit.lua must load after Main.lua")
end
...
local function Size(frame, width, height, ...)
	local w = LilyUI:Scale(width)
	frame:SetSize(w, (height and LilyUI:Scale(height)) or w, ...)
end

local function Width(frame, width, ...)
	frame:SetWidth(LilyUI:Scale(width), ...)
end

local function Height(frame, height, ...)
	frame:SetHeight(LilyUI:Scale(height), ...)
end

local function Point(obj, arg1, arg2, arg3, arg4, arg5, ...)
	if not arg2 then arg2 = obj:GetParent() end

	if type(arg2)=='number' then arg2 = LilyUI:Scale(arg2) end
	if type(arg3)=='number' then arg3 = LilyUI:Scale(arg3) end
	if type(arg4)=='number' then arg4 = LilyUI:Scale(arg4) end
	if type(arg5)=='number' then arg5 = LilyUI:Scale(arg5) end
...
local function NudgePoint(obj, xAxis, yAxis, noScale, pointValue, clearPoints)
	...
	local x = (noScale and xAxis) or LilyUI:Scale(xAxis)
	local y = (noScale and yAxis) or LilyUI:Scale(yAxis)
...
local function PointXY(obj, xOffset, yOffset, noScale, pointValue, clearPoints)
	local x = xOffset and ((noScale and xOffset) or LilyUI:Scale(xOffset))
	local y = yOffset and ((noScale and yOffset) or LilyUI:Scale(yOffset))
...
local function SetOutside(obj, anchor, xOffset, yOffset, anchor2, noScale)
	...
	local x = (noScale and xOffset) or LilyUI:Scale(xOffset)
	local y = (noScale and yOffset) or LilyUI:Scale(yOffset)
