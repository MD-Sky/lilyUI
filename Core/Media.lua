...
LSM:Register("statusbar","Lily", "Interface\\AddOns\\lilyUI\\Media\\Lily.tga")
LSM:Register("statusbar","Neph", "Interface\\AddOns\\lilyUI\\Media\\Lily.tga") -- legacy alias
LSM:Register("font","Expressway", [[Interface\AddOns\lilyUI\Fonts\Expressway.TTF]])

function LilyUI:GetGlobalFont()
    local fontName = self.db.profile.general.globalFont or "Expressway"
    return LSM:Fetch("font", fontName) or [[Interface\AddOns\lilyUI\Fonts\Expressway.TTF]]
end

function LilyUI:GetGlobalTexture()
    local textureName = self.db.profile.general.globalTexture or "Lily"
...
function LilyUI:GetTexture(overrideTexture)
...
function LilyUI:ApplyGlobalFont()
    ...
    -- Always apply fonts to LilyUI's own elements (cooldown viewers, target auras, etc.)
    if not self._cooldownFontHooked then
        local function IdentifyCooldownSource(cooldownFrame)
            ...
            if hasIcon or hasCooldownRef then
                -- LilyUI CustomIcons: per-icon cooldown text settings
                ...
                if viewerFrame then
                    ...
                    if viewerName then
                        ...
                        elseif viewerName == "BuffIconCooldownViewer" then
                            ...
                            elseif viewerName == "LilyUI_Target" then
                        ...
                        -- Also check if viewerFrame's parent is LilyUI_Target (in case of nested frames)
                        local targetFrame = viewerFrame:GetParent()
                        if targetFrame then
                            local targetFrameName = targetFrame:GetName()
                            if targetFrameName == "LilyUI_Target" then
        ...
        if LilyUI.UnitFrames and LilyUI.UnitFrames.UpdateTargetAuras then
            local originalUpdateTargetAuras = LilyUI.UnitFrames.UpdateTargetAuras
            LilyUI.UnitFrames.UpdateTargetAuras = function(frame, ...)
        ...
        if currentFontPath then
            local function IdentifyCooldownSource(cooldownFrame)
                ...
                -- For target auras: iconFrame is parent of cooldown, and iconFrame's parent is LilyUI_Target
                local iconFrame = parent
                local hasIcon = iconFrame.icon or iconFrame.Icon
                local hasCooldownRef = iconFrame.cooldown == cooldownFrame
